...
export default function LoopFooter({ menuItems, cosialIcons }) {
  return (
    <footer className="bg-black p-6">
      <div className="max-w-6xl mx-auto px-6 py-10">
        <div className="text-white flex flex-col gap-6 justify-center md:justify-between md:items-start md:flex-row">
          <nav className="flex flex-col justify-center items-center gap-4 md:items-start md:justify-start">
            <svg
              width="192"
              height="32"
              ...
              {!!menuItems.length && menuItems.map(menuItem => (
                <li key={menuItem} className="group">
                  <a className="block" href={menuItem === 'Home' ? '/' : '#'}>
                    <span className="block font-light text-base font-josefin">{menuItem}</span>
          ...
          <div className="flex flex-col items-center md:items-start gap-4">
            <div className="flex gap-4 h-8">
              {!!cosialIcons.length && cosialIcons.map(({link, icon}, idx) => (
                <a key={idx + '--d9f7asdfd98sfdsf'} href={link} className="block w-6 h-6 group">
                  <span className="[&>svg]:duration-200 [&>svg]:group-hover:scale-110">{icon}</span>
                </a>
              ))}
            </div>
            <p className="font-light font-josefin text-center md:text-left">&copy; {new Date().getFullYear()} Loopstudios. All Rights Reserved</p>
          </div>
        </div>
      </div>
    </footer>
