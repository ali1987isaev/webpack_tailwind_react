...
export default function LoopHeader({ menuItems, setShowMenu, showMenu }) {
  return (
    <header className={`container mx-auto px-6 py-12 absolute z-50 left-0 right-0 top-0 ${showMenu ? 'bg-black' : ''}`}>
      <nav className="flex items-center justify-between text-white">
        ...
        <ul className="hidden md:flex items-center gap-8">
          {!!menuItems.length && menuItems.map(menuItem => (
            <li key={menuItem} className="group">
              <a className="block" href={menuItem === 'Home' ? '/' : '#'}>
                <span className="block font-josefin text-base">{menuItem}</span>
                <span className="block border-b border-white w-0 opacity-0 -translate-y-1 duration-300 group-hover:translate-y-0 group-hover:opacity-100 group-hover:w-full"></span>
              </a>
            </li>
          ))}
        </ul>
        <button onClick={() => setShowMenu(!showMenu)} type="button" className="text-white cursor-pointer md:hidden">
          <div className="relative w-6 h-6 transition-all duration-300"> 
            <div className={`absolute transition-all duration-300 h-0.5 bg-white left-0 w-full top-0 ${showMenu ? 'rotate-45 translate-y-2' : ''}`}></div>
            <div className={`absolute transition-all duration-100 h-0.5 bg-white left-0 w-full top-0 translate-y-2 ${showMenu ? 'opacity-0' : ''}`}></div>
            <div className={`absolute transition-all duration-300 h-0.5 bg-white left-0 w-full top-0 ${showMenu ? '-rotate-45 translate-y-2' : 'translate-y-4'}`}></div>
          </div>
        </button>
      </nav>
    </header>
